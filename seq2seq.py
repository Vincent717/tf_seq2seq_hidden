...
def embedding_attention_decoder(decoder_inputs,   # encoder outputs
                                initial_state,    # encoder states
                                attention_states,
                                cell,
                                num_symbols,
                                embedding_size,
...
def embedding_attention_seq2seq(encoder_inputs,
                                ...
                                scope=None,
                                initial_state_attention=False,
                                return_input_hidden_states=False):
  ...
  with variable_scope.variable_scope(
    ...
    encoder_cell = core_rnn_cell.EmbeddingWrapper(
        ...
        embedding_classes=num_encoder_symbols,
        embedding_size=embedding_size)
    encoder_outputs, encoder_state = core_rnn.static_rnn(   # encoder_state is what we want!
        encoder_cell, encoder_inputs, dtype=dtype)
    ...
    if isinstance(feed_previous, bool):
      outputs, states = embedding_attention_decoder(   # after encode, push all staff to decoder function
            decoder_inputs,
            encoder_state,
            attention_states,
            cell,
            ...
            feed_previous=feed_previous,
            initial_state_attention=initial_state_attention)
      ### see if we want to return encoder hidden states
      if return_input_hidden_states:
    ...
    if return_input_hidden_states:
      ...
      return outputs_and_state[:outputs_len]
...
def model_with_buckets(encoder_inputs,
  ...
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(
        ...
        ### in our case, seq2seq = embedding_attention_seq2seq()
        if return_input_hidden_states:
          bucket_outputs, bucket_states, bucket_input_states = seq2seq(encoder_inputs[:bucket[0]],
                                                                       decoder_inputs[:bucket[1]])
          output_states.append(bucket_states)
          input_states.append(bucket_input_states)
        else:
  ...
  if return_hidden_states:
    return outputs, losses, input_states, output_states
  else:
